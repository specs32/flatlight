...
long readVcc() {
  ...
  result = 1300000L / result;         // Calculate Vcc <--- ADJUST since it can be 10 % off (default 1.23V) (1350000L
...
while(temp > 50){
    ...
    temp=Thermistor(val); 
      ...
      // if temperatur falls blow 35°C go to 50% brightness
...
void LED() {
  ...
  lightMode = lightMode + 1;
    ...
    if (lightMode == 1){
      ...
      LEDstate = ((power * 121)-10);
      fade();
      previousLEDstate = LEDstate;
    }
    
    else if (lightMode == 4){
      LEDstate = ((power * 255)-10); // a little offset for the cree xp-something leds, because they really get hot the small thinggies
      fade();
      previousLEDstate = LEDstate;
    }
}
